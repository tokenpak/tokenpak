...
class TelemetryCollector:
    ...
    def _check_flush_timeout(self):
        if self.pending_events and (time.time() - self.last_flush_time) >= self.config.batch_timeout_seconds:
            self._flush_batch()
        ...
        try:
            with open(self.config.state_file, 'r') as f:
                ...
                for key, s in data.get("file_states", {}).items():
                    self.file_states[key] = FileState(path=Path(s["path"]),
    ...
    def _save_state(self):
        if not self.config.state_file:
            return
        try:
            data = {"file_states": {k: {"path": str(s.path), "last_position": s.last_position,
                "last_modified": s.last_modified, "events_sent": s.events_sent}
...
def create_collector(watch_paths: list[str], ingest_url: str = "http://localhost:17888/v1/telemetry/ingest",
                     backfill: bool = False, state_file: Optional[str] = None) -> TelemetryCollector:
    """Factory: create a TelemetryCollector from a config dict."""
    return TelemetryCollector(CollectorConfig(watch_paths=[Path(p) for p in watch_paths],
        ingest_url=ingest_url, backfill_on_start=backfill,
