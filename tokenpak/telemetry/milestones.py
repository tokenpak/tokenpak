...
_MILESTONE_DEFS = (
    [("savings_usd", t, f"You've saved over ${t:,.0f} with TokenPak")
     for t in SAVINGS_THRESHOLDS] +
    [("compression_pct", t, f"Compression improved to {t}%+")
     for t in COMPRESSION_IMPROVEMENTS] +
    [("retry_pct_below", t, f"Retry rate dropped below {t}%")
     for t in RETRY_DROP_THRESHOLDS] +
    [("request_count", t, f"First {t:,} requests tracked")
     for t in REQUEST_COUNT_THRESHOLDS]
)
...
def _efficiency_score(compression_pct: float, error_pct: float, retry_pct: float) -> float:
    ...
    Clamp to [0, 100].
    """
    score = (min(compression_pct, 100) * 0.40 +
...
def get_milestone_history(db_path: Optional[str] = None) -> list[Milestone]:
    ...
    try:
        ...
        return [
            Milestone(id=int(r[0]), milestone_type=str(r[1]), threshold=float(r[2]),
                      label=str(r[3]), reached_at=float(r[4]), acknowledged=bool(r[5]))
            for r in cur.fetchall()
        ]
    finally:
        conn.close()
...
def acknowledge_milestone(milestone_id: int, db_path: Optional[str] = None) -> bool:
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE tp_milestones SET acknowledged = 1 WHERE id = ?",
            (milestone_id,)
        )
        conn.commit()
...
def check_and_create_milestones(db_path: Optional[str] = None) -> list[Milestone]:
    ...
    try:
        ...
        # Lifetime savings
        cur.execute(
            """SELECT COALESCE(SUM(c.savings_total), 0)
               FROM tp_costs c
               JOIN tp_events e ON c.trace_id = e.trace_id
        ...
        for m_type, threshold, label in _MILESTONE_DEFS:
            ...
            if reached:
                ...
                new_milestones.append(Milestone(
                    id=mid, milestone_type=m_type, threshold=threshold,
                    label=label, reached_at=now, acknowledged=False
                ))
