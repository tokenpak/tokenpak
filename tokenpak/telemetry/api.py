...
try:
    ...
    FASTAPI_AVAILABLE = False; APIRouter = None; Query = None

from tokenpak.telemetry.query import get_cost_summary, get_daily_trend, get_model_usage, get_recent_events, get_savings_report
...
def _to_json(obj):
    if isinstance(obj, list): return [asdict(i) if hasattr(i, "__dataclass_fields__") else i for i in obj]
...
if FASTAPI_AVAILABLE:
    ...
    def api_health():
        ...
        import sqlite3
        db = _get_db_path(); exists = db.exists()
        sz, ev, co = 0.0, 0, 0
        if exists:
            sz = db.stat().st_size / 1024 / 1024
            try:
                c = sqlite3.connect(str(db)); cur = c.cursor()
                cur.execute("SELECT COUNT(*) FROM tp_events"); ev = cur.fetchone()[0]
                cur.execute("SELECT COUNT(*) FROM tp_costs"); co = cur.fetchone()[0]
                c.close()
            except Exception as e:
                return {"status": "error", "db_path": str(db), "db_exists": exists, "db_size_mb": round(sz, 2), "error": str(e), "event_count": 0, "cost_count": 0}
        return {"status": "ok" if exists else "no_db", "db_path": str(db), "db_exists": exists, "db_size_mb": round(sz, 2), "event_count": ev, "cost_count": co}
    ...
    def api_savings_history(days: int = Query(default=90, ge=1, le=365)):
        ...
        return [
            {"date": p.date, "daily_savings": p.daily_savings, "cumulative_savings": p.cumulative_savings}
    ...
    def api_milestones(all: bool = Query(default=False)):
        ...
        from tokenpak.telemetry.milestones import get_pending_milestones, get_milestone_history
        fn = get_milestone_history if all else get_pending_milestones
        return [
            {"id": m.id, "milestone_type": m.milestone_type, "threshold": m.threshold,
             "label": m.label, "reached_at": m.reached_at, "acknowledged": m.acknowledged}
    ...
    def api_check_milestones():
        ...
        return [
            {"id": m.id, "milestone_type": m.milestone_type, "threshold": m.threshold,
             "label": m.label, "reached_at": m.reached_at}
            for m in new_ms
        ]
...
def create_app():
    ...
    if not FASTAPI_AVAILABLE: raise ImportError("FastAPI required")
