...
class QueryBuilder:
    ...
    INDEXED_COLUMNS = {
        ...
        'actual_cost', 'savings_amount', 'latency_ms',
        'provider', 'model', 'status', 'user_id'
    }
    
    # Columns safe for filtering
    FILTERABLE_COLUMNS = {
        'provider', 'model', 'status', 'user_id', 'session_id',
        'actual_cost', 'savings_amount', 'latency_ms',
    ...
    def add_sort(self, spec: SortSpec) -> 'QueryBuilder':
        ...
        if spec.column not in self.INDEXED_COLUMNS:
            raise ValueError(f'Column not indexed: {spec.column}. Queries will be slow. Use: {self.INDEXED_COLUMNS}')
