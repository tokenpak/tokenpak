"""DEPRECATED — `tokenpak template` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.template`` and is
installed via:
...
template`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.template: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak template` command requires a Pro subscription.\n"
...
template_cmd = _upgrade_stub


__all__ = ["template_cmd"]
