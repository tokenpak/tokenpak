"""DEPRECATED — `tokenpak last` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.last`` and is
installed via:
...
last`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.last: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak last` command requires a Pro subscription.\n"
...
last_cmd = _upgrade_stub
run = _upgrade_stub


__all__ = ["last_cmd", "run"]
