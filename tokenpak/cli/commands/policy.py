"""DEPRECATED — `tokenpak policy` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.policy`` and is
installed via:
...
policy`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.policy: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak policy` command requires a Enterprise subscription.\n"
