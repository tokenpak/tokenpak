"""DEPRECATED — `tokenpak replay` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.replay`` and is
installed via:
...
replay`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.replay: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak replay` command requires a Pro subscription.\n"
...
build_replay_parser = _upgrade_stub


__all__ = ["build_replay_parser"]
