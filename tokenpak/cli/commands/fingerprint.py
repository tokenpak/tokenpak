"""DEPRECATED — `tokenpak fingerprint` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.fingerprint`` and is
installed via:
...
fingerprint`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.fingerprint: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak fingerprint` command requires a Pro subscription.\n"
...
fingerprint_cmd = _upgrade_stub


__all__ = ["fingerprint_cmd"]
