"""DEPRECATED — `tokenpak budget` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.budget`` and is
installed via:
...
budget`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.budget: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak budget` command requires a Team subscription.\n"
...
run_budget_cmd = _upgrade_stub


__all__ = ["run_budget_cmd"]
