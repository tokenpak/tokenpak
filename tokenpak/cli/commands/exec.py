"""DEPRECATED — `tokenpak run` moved to tokenpak-paid (2026-04-21).

This module is a stub left behind by TPS-11. The real implementation
now lives in ``tokenpak_paid.commands._impls.exec`` and is
installed via:
...
run`` to the real paid implementation when it is installed.
"""
...
_warnings.warn(
    "tokenpak.cli.commands.exec: implementation moved to "
...
def _upgrade_stub(*args, **kwargs):
    ...
    print(
        "⚠ The `tokenpak run` command requires a Team subscription.\n"
