...
PREMADE_MACROS: Dict[str, Dict[str, Any]] = {
    "morning-standup": {
        ...
        "steps": [
            ...
            {"name": "model_usage", "cmd": "tokenpak stats --today --json", "label": "📊 Model Usage"},
    ...
    "pre-deploy": {
        ...
        "steps": [
            {"name": "budget_check", "cmd": "tokenpak budget status --json", "label": "💳 Budget Headroom"},
            {"name": "stats_snapshot", "cmd": "tokenpak stats --snapshot --json", "label": "📸 Stats Snapshot"},
        ],
    },
    "weekly-report": {
        "description": "7-day cost, token savings, model usage breakdown, and top recipes",
        "steps": [
            {"name": "weekly_cost", "cmd": "tokenpak stats --days 7 --json", "label": "📅 7-Day Cost & Usage"},
            {"name": "savings", "cmd": "tokenpak stats --days 7 --savings --json", "label": "💡 Token Savings"},
            {"name": "top_recipes", "cmd": "tokenpak recipes list --top --json", "label": "🍳 Top Recipes"},
        ],
    },
}
...
class PremadeMacroRunner:
    ...
    def run(self, name: str, json_output: bool = False) -> Dict[str, Any]:
        ...
        """
        if name not in PREMADE_MACROS:
            raise ValueError(f"Unknown macro: '{name}'. Available: {', '.join(PREMADE_MACROS.keys())}")
