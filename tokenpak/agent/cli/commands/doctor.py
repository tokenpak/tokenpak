...
def run_doctor(
    ...
    if health is not None:
        ...
        _record(
            ...
            detail=f"compilation_mode={mode} requests={requests_total}",
        )
    else:
...
try:
    ...
    def doctor_cmd(
        ...
        if fleet:
            ...
            rc = run_doctor(fix=fix, output_json=output_json, verbose=verbose)
        sys.exit(rc)
...
DEFAULT_FLEET_CONFIG = {
    "agents": [
        {"name": "agent-1", "host": "host-1", "user": "agent1"},
        {"name": "agent-2", "host": "host-2", "user": "agent2"},
        {"name": "agent-3", "host": "host-3", "user": "agent3"},
    ]
}
