...
def list_cmd(filter_status, tags, filter_type, limit, output_json):
    """List workflows. Use --type proxy to filter to proxy-type workflows."""
    mgr = get_manager()
    status = WorkflowStatus(filter_status) if filter_status else None
    # Combine explicit tags with --type shortcut
    all_tags = list(tags)
    if filter_type:
        all_tags.append(filter_type)
    records = mgr.list_workflows(status=status, tags=all_tags if all_tags else None, limit=limit)
    if output_json:
        click.echo(json.dumps([r.to_dict() for r in records], indent=2)); return
    if not records:
        type_msg = f" (type={filter_type})" if filter_type else ""
        click.echo(f"No workflows found{type_msg}."); return
    click.echo(SEP)
...
def status_cmd(wf_id, output_json):
    ...
    click.echo(f"  Progress   : {wf.completion_pct():.0f}%  ({sum(1 for s in wf.steps if s.is_done())}/{len(wf.steps)} steps)")
...
def resume_cmd(wf_id):
    ...
    else: click.echo("   All pending steps blocked or none remain.")
...
def cancel_cmd(wf_id, yes):
    ...
    mgr = get_manager()
    wf = _resolve(mgr, wf_id)
    if not yes: click.confirm(f"Cancel workflow '{wf.name}'?", abort=True)
    wf = mgr.cancel(wf.id)
