...
# ---------------------------------------------------------------------------
# Compatibility shims: ProxyServer, GracefulShutdown and helpers.
# Tests import these names from tokenpak.proxy.server.  The canonical
# implementations live in tokenpak.agent.proxy.server; these are lightweight
# copies/stubs so that the proxy.server module remains self-contained.
# ---------------------------------------------------------------------------
import hashlib as _hashlib
import time as _time
from contextlib import contextmanager as _contextmanager
from typing import Any as _Any, Dict as _Dict, Generator as _Generator, Optional as _Optional
...
class GracefulShutdown:
    ...
    @_contextmanager
    def track_request(self) -> _Generator[None, None, None]:
        with self._lock:
            self._in_flight += 1
...
def _new_session() -> _Dict[str, _Any]:
    return {
        "requests": 0,
        "input_tokens": 0,
        "sent_input_tokens": 0,
...
def _compute_stable_prefix_hash(body: _Optional[bytes]) -> str:
    if not body:
        return ""
    try:
        ...
        if isinstance(system, str):
            ...
            return ""
        return _hashlib.sha256(stable_text.encode("utf-8")).hexdigest()[:16]
    except Exception:
        return ""
...
def auto_detect_upstream(request_headers: dict) -> str:
    ...
    if request_headers.get("x-goog-api-key") or request_headers.get("X-Goog-Api-Key"):
        return "https://generativelanguage.googleapis.com"
    return "https://api.anthropic.com"


class ProxyServer:
    """TokenPak HTTP proxy server (compatibility shim — see tokenpak.agent.proxy.server)."""

    def __init__(
        self,
        host: str = "127.0.0.1",
        port: _Optional[int] = None,
        compilation_mode: _Optional[str] = None,
        request_hook=None,
        shutdown_timeout: _Optional[float] = None,
    ):
        import os as _os
        self.host = host
        self.port = port or int(_os.environ.get("TOKENPAK_PORT", "8766"))
        self.compilation_mode = compilation_mode or _os.environ.get("TOKENPAK_MODE", "hybrid")
    ...
    def start(self) -> None:
        raise NotImplementedError("Use tokenpak.agent.proxy.server.ProxyServer for production use")

    def stop(self) -> None:
        self.shutdown.begin()
        self.shutdown.wait_for_drain(timeout=self.shutdown_timeout)
