...
class TestImageType:
    def test_import(self):
        from tokenpak.processors.image import ImageType
...
class TestCompressedImage:
    def test_import(self):
        from tokenpak.processors.image import CompressedImage
        assert CompressedImage is not None

    @requires_pillow
    def test_fields(self):
        from tokenpak.processors.image import CompressedImage, ImageType
        ci = CompressedImage(
            data=b"abc",
            original_size=100,
            compressed_size=50,
...
class TestClassifyNoPillow:
    def test_raises_import_error_when_pillow_absent(self):
        with mock.patch("tokenpak.processors.image._PILLOW_AVAILABLE", False):
            from tokenpak.processors.image import classify
...
class TestCompressNoPillow:
    def test_raises_import_error_when_pillow_absent(self):
        with mock.patch("tokenpak.processors.image._PILLOW_AVAILABLE", False):
            from tokenpak.processors.image import compress
            with pytest.raises(ImportError, match="pip install tokenpak"):
...
class TestImageProcessorNoPillow:
    def test_passthrough_returns_original_bytes(self):
        with mock.patch("tokenpak.processors.image._PILLOW_AVAILABLE", False):
            from tokenpak.processors.image import ImageProcessor
            ip = ImageProcessor()
            raw = b"\x89PNG\r\n\x1a\n" + b"\x00" * 50
...
class TestProcessorsDict:
    def test_image_in_processors(self):
        from tokenpak.processors import PROCESSORS
...
class TestPillowFlag:
    def test_flag_is_bool(self):
        import tokenpak.processors.image as img_mod
