...
def check_alerts() -> list[tuple[AlertRule, Optional[float]]]:
    ...
    Any alert that fires is also dispatched to configured channels via
    tokenpak.alerts.channels.dispatch_alert (fire-and-forget background thread).
    """
    config = load_config()
    ...
    if fired:
        from tokenpak.alerts.channels import dispatch_alert
        for rule, value in fired:
            msg = rule.message.format(value=value) if value is not None else rule.message
            dispatch_alert(event=rule.name, severity="warning", message=msg)
