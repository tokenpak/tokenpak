...
try:
    ...
    _GLOSSARY_DATA_GLOBAL: dict = _json_mod.loads(_TERM_CARDS_PATH.read_text()) if _TERM_CARDS_PATH.exists() else {}
except Exception:
...
def _detect_cost_spike(storage) -> list:
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT SUM(estimated_cost) as total FROM requests WHERE date(timestamp) = ?",
            (today,)
        )
        row = cursor.fetchone()
        ...
        if last_week_avg > 0 and today_cost > 2 * last_week_avg:
            n = _add_notification("alert", "Cost Spike Detected", f"Today's cost (${today_cost:.2f}) is > 2x the average")
            return [n]
        return []
...
def _check_cache_miss_rate(storage) -> list:
    ...
    try:
        ...
        if total > 0:
            ...
            if miss_rate > 0.8:
                n = _add_notification("warning", "High Cache Miss Rate", f"Cache miss rate is {miss_rate:.1%}")
                return [n]
        return []
    except Exception as e:
...
def _safe_list_traces(storage: TelemetryDB, limit: int, provider: Optional[str],
                      model: Optional[str], agent: Optional[str]) -> list:
    """List traces, returning empty list on error."""
    try:
        return storage.list_traces(limit=limit, offset=0,
                                   provider=provider, model=model,
                                   agent_id=agent, since_ts=None)
    except TypeError:
...
def _build_glossary_context() -> dict:
    ...
    TERM_LABELS = {
        "baseline_cost":"Baseline Cost","actual_cost":"Actual Cost","savings":"Savings",
        "savings_pct":"Savings %","compression_ratio":"Compression Ratio",
        "error_rate":"Error Rate","retry_rate":"Retry Rate",
        "latency_avg":"Latency (Avg)","latency_p95":"Latency (p95)","latency_p99":"Latency (p99)",
        "raw_tokens":"Raw Tokens","final_tokens":"Final Tokens","reconciled":"Reconciled",
        "estimated":"Estimated","cache_tokens":"Cache Tokens","request_count":"Request Count",
    ...
    UNIT_RULES = {
        ...
        "compression_ratio":{"example":"1.4×","pattern":"X.X× (multiplication sign, raw÷final)"},
...
def create_dashboard_router(
    ...
    async def finops(
        ...
        try:
            from tokenpak.telemetry.milestones import (
                get_savings_summary, get_savings_history, get_pending_milestones
            )
            savings_summary = await loop.run_in_executor(_executor, get_savings_summary, None)
            savings_history = await loop.run_in_executor(_executor, lambda: get_savings_history(days=days))
            pending_milestones = await loop.run_in_executor(_executor, get_pending_milestones, None)
        except Exception:
            from types import SimpleNamespace
            savings_summary = SimpleNamespace(
                lifetime_savings=0, savings_30d=0, savings_7d=0,
                trend_pct=0, efficiency_score=0, compression_pct=0, total_requests=0
            )
            savings_history = []
            pending_milestones = []
    ...
    async def export_csv(
        ...
        filters = {"days": days, "provider": provider, "model": model, "agent": agent, "status": status}
    ...
    async def export_trace(trace_id: str):
        ...
        if not trace:
            return Response(content='{"error": "Trace not found"}', status_code=404, media_type="application/json")
    ...
    async def executive_summary_api(
        ...
        summary = await loop.run_in_executor(
            _executor, _generate_executive_summary, rollups, days, provider or None, model or None, format
        )
    ...
    async def settings_view(request: Request, section: str = "personal"):
        ...
        valid_sections = [
            "personal", "dashboard", "data", "pricing",
            "alerts", "access", "integrations", "system"
        ]
...
def _generate_executive_summary(rollups, days: int, provider: str = None, model: str = None, format: str = "paragraph"):
    """Generate executive summary in paragraph or bullet format."""
    import datetime
    ...
    if format == "bullets":
        ...
        if risks:
            ...
            lines.append(f"• **Status:** No anomalies detected")
...
def _build_csv_export(traces, filters):
    ...
    # Header row
    writer.writerow([
        "trace_id", "timestamp", "provider", "model",
        "raw_input_tokens", "compressed_input_tokens", "output_tokens",
        "baseline_cost", "actual_cost", "savings", "savings_pct",
        "latency_ms", "status", "data_source"
    ])
