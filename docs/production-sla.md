...
> **Note:** Targets reflect a 4GB-RAM reference machine running the Python
> HTTPServer with a 20-worker bounded pool. Production deployment with the
> asyncio server (`server_async.py`) should achieve p99 < 20ms.

## Measured Performance (2026-03-24)

Benchmark run on reference hardware, 100 req/sec sustained for 5s:
