...
**Self-hosted:** Your data never leaves your infrastructure. TokenPak runs on your machine or server and only talks to the provider's API. No external logging, no analytics, no data storage. Responses are only cached in-memory (configurable TTL).
...
Yes. TokenPak logs all requests (model, prompt hash, response length, cost, latency) to local files and stdout. Self-hosted users can integrate their own logging backend via webhooks.
