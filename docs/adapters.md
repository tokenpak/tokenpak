...
Adapters are converters between your code's request/response format and each provider's native format. TokenPak includes **5 built-in adapters**.
...
| `openai_chat` | OpenAI Chat API | ✅ | GPT-4o, GPT-5.x |
| `openai_responses` | OpenAI Responses (Legacy) | ⚠️ Deprecated | Older OpenAI integrations |
...
Routes requests to OpenAI's Chat API (GPT-4o, GPT-5.x).
...
response = client.messages.create(
    model="gemini-2.5-flash",  # Gemini models support multimodal natively
    max_tokens=200,
    messages=[
        {
            "role": "user",
...
### Use OpenAI Chat if:
- ✅ Primary provider is OpenAI (GPT-4o, GPT-5.x)
