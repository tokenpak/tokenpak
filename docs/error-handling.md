...
Failed to connect to http://localhost:8766
```
...
```bash
# Detailed logs
TOKENPAK_LOG_LEVEL=DEBUG tokenpak start

# Write to file
tokenpak start --log-file /tmp/tokenpak.log
```
...
```bash
# Last 100 requests
curl http://localhost:8766/logs?limit=100

# Requests to Anthropic only
curl 'http://localhost:8766/logs?provider=anthropic'

# Requests with errors
curl 'http://localhost:8766/logs?status=error'
```
...
|-------|-------|----------|
| `ConnectionRefusedError` | Proxy not running | Start `tokenpak start` |
