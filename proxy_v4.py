...
Env vars:
    ...
    TOKENPAK_COMPACT_THRESHOLD_TOKENS (default: 1500) — skip compaction below this
    ...
    TOKENPAK_BUDGET_CONTROLLER (default: 1) — enforce token budget limits per request
...
COMPACT_THRESHOLD_TOKENS = _cfg("compression.threshold_tokens", 1500, "TOKENPAK_COMPACT_THRESHOLD_TOKENS", int)  # Flipped 2026-04-07 — see TRIX-01 / pmgtm initiative
...
BUDGET_MONTHLY_USD: Optional[float] = _cfg("budget.monthly_usd", None, "TOKENPAK_BUDGET_MONTHLY_USD", float)
...
BUDGET_CONTROLLER_ENABLED: bool = _cfg("features.budget_controller", True, "TOKENPAK_BUDGET_CONTROLLER", bool)  # Flipped 2026-04-07 — see TRIX-01 / pmgtm initiative
...
# ---------------------------------------------------------------------------
# RecipeEngine — eager load OSS recipes on serve start
# Flipped 2026-04-07 — see TRIX-01 / pmgtm initiative
# ---------------------------------------------------------------------------
_RECIPE_ENGINE_MODULE_PATH = str(Path.home() / "vault" / "Projects" / "ocp-protocol" / "packages" / "pypi")
if _RECIPE_ENGINE_MODULE_PATH not in sys.path:
    sys.path.insert(0, _RECIPE_ENGINE_MODULE_PATH)
try:
    from tokenpak.agent.compression.recipes import RecipeEngine as _EagerRecipeEngine
    _EAGER_RECIPE_ENGINE = _EagerRecipeEngine()
    print("  📖 RecipeEngine loaded (OSS recipes ready)")
except Exception as _re_err:
    _EagerRecipeEngine = None  # type: ignore[assignment,misc]
    _EAGER_RECIPE_ENGINE = None
    print(f"  ⚠️  RecipeEngine unavailable: {_re_err}")
...
# ---------------------------------------------------------------------------
# Monthly USD spend cache — avoids DB hit on every request (TTL: 60 s)
# ---------------------------------------------------------------------------
_MONTHLY_SPEND_CACHE: dict = {"usd": 0.0, "ts": 0.0}
_MONTHLY_SPEND_CACHE_TTL: float = 60.0


def _get_cached_monthly_spend_usd() -> float:
    """Return current-month spend in USD, refreshing from DB at most once per 60 s."""
    now = time.time()
    if now - _MONTHLY_SPEND_CACHE["ts"] < _MONTHLY_SPEND_CACHE_TTL:
        return _MONTHLY_SPEND_CACHE["usd"]
    try:
        spent = MONITOR.get_monthly_cost()
    except Exception:
        spent = _MONTHLY_SPEND_CACHE["usd"]  # use stale value on error
    _MONTHLY_SPEND_CACHE["usd"] = spent
...
class Monitor:
    ...
    def get_monthly_cost(self) -> float:
        """Return total estimated_cost for the current calendar month (UTC)."""
        conn = sqlite3.connect(self.db_path)
        row = conn.execute("""
        ...
        if should_log and is_messages and body:
            ...
            try:
                ...
                if pipeline_enabled:
                    ...
                    # Phase 0.2.B: Monthly USD budget enforcement
                    # Reads budget.monthly_usd from config; default=None (unlimited).
                    # Uses a 60-second TTL cache so the DB is not hit on every request.
                    if BUDGET_MONTHLY_USD is not None:
                        try:
                            _bc_monthly = _get_budget_controller()
                            if _bc_monthly is not None:
                                _monthly_spent = _get_cached_monthly_spend_usd()
                                _bc_result = _bc_monthly.check(BUDGET_MONTHLY_USD, _monthly_spent)
                                if _bc_result.exceeded:
                                    SESSION["budget_blocked_total"] += 1
                                    self._send_json({
                                        "error": {
                                            "type": "budget_exceeded",
                                            "message": (
                                                f"Monthly budget of ${BUDGET_MONTHLY_USD:.2f} exceeded. "
                                                f"Spent: ${_monthly_spent:.4f}"
                                            ),
                                            "limit_usd": _bc_result.limit_usd,
                                            "spent_usd": _bc_result.spent_usd,
                                            "reset_at": _bc_result.reset_at,
                                        }
                                    }, status=429)
                                    return
                        except Exception as _bc_monthly_err:
                            pass  # fail-open: never block requests due to budget check error
