...
# Readiness flag — True once server is accepting requests; False during shutdown.
...
def _startup_preflight(port: int) -> None:
    """
    Run pre-flight validation checks before binding the port.

    Raises SystemExit on any fatal condition so the process never reaches
    serve_forever() in a broken state.
    """
    errors: list[str] = []

    # 1. Port availability — detect by attempting a client connect.
    #    Using connect() (not bind+SO_REUSEADDR) is the reliable way to tell
    ...
    try:
        ...
        _probe.close()
        _port_in_use = True  # connect succeeded → a server is listening
    except (ConnectionRefusedError, socket.timeout, OSError):
        pass  # refused or timeout → port is free

    if _port_in_use:
        errors.append(
            f"Port {port} is already in use.\n"
            f"  Fix: stop the existing process, or set TOKENPAK_PORT to a free port.\n"
            f"  Check: lsof -i :{port}  or  ss -tlnp | grep {port}"
        )

    # 2. Config validation (already run below, but surface as fatal if critical keys missing)
    # validate_tokenpak_config() handles auto-correct; no blocking here.

    if errors:
        print("\n[startup] ❌ Pre-flight checks FAILED — proxy will not start:\n")
        for err in errors:
            print(f"  • {err}")
        print()
        sys.exit(1)
...
def main():
    ...
    # Pre-flight: validate port availability before binding anything
    ...
    # Proxy workflow tracking — startup dangling workflow check (kill -9 recovery)
    try:
        ...
        if dangling:
            print(f"[proxy_workflow] ⚠️  {len(dangling)} incomplete proxy workflow(s) from prior run (kill -9 recovery):")
            for wf in dangling[:5]:
                running_step = next((s["name"] for s in wf["steps"] if s["status"] == "running"), "—")
                print(f"  • {wf['id'][:8]}… step={running_step}")
            print("[proxy_workflow] ✅ Recovered — stale workflows marked as interrupted")
    except Exception as _e:
...
║                                                                  ║
║  Endpoints:  /health  /ready  /stats  /recent  /vault             ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝
    """)
    ...
    def _handle_signal(signum, frame):
        ...
        _proxy_ready = False   # /ready returns 503 immediately
        _shutdown_event.set()
    ...
    # Mark proxy as ready — /ready returns 200 from this point
    global _proxy_ready
    _proxy_ready = True
    print(f"[startup] ✅ TokenPak proxy ready — port={port} version={__import__('tokenpak').__version__}")

    server.serve_forever()

    # --- Drain in-flight requests (up to 30s — matches systemd TimeoutStopSec) ---
    _SHUTDOWN_DRAIN_TIMEOUT = int(os.getenv("TOKENPAK_SHUTDOWN_TIMEOUT", "30"))
    drain_timeout = _SHUTDOWN_DRAIN_TIMEOUT
    with _active_request_lock:
