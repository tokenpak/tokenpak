...
# CCI-08: Shadow A/B model comparison logging — default OFF (adds upstream cost)
SHADOW_AB_ENABLED: bool = _cfg("features.shadow_ab", False, "TOKENPAK_SHADOW_AB_ENABLED", bool)
SHADOW_AB_PROVIDER: str = _cfg("features.shadow_ab_provider", "anthropic-haiku", "TOKENPAK_SHADOW_PROVIDER", str)
SHADOW_AB_SAMPLE_RATE: int = _cfg("features.shadow_ab_sample_rate", 5, "TOKENPAK_SHADOW_AB_SAMPLE_RATE", int)
SHADOW_AB_STORE_CONTENT: bool = _cfg("features.shadow_ab_store_content", True, "TOKENPAK_SHADOW_AB_STORE_CONTENT", bool)
...
def _run_shadow_request(
    ...
    try:
        ...
        # Never stream the shadow request
        req_data["stream"] = False
        shadow_body = json.dumps(req_data).encode()

        # Build headers — forward the API key from env
        ...
        try:
            ...
            pass  # shadow upstream unreachable — log with zero tokens

        similarity = _compute_similarity(primary_response_text, shadow_response_text)

        # Hash content if store_content=False
        if store_content:
        ...
        try:
            ...
            pass  # fail-open: never break anything over a shadow write
    except Exception:
...
class ForwardProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/dashboard/shadow-ab":
            # CCI-08: Shadow A/B comparison panel
            try:
                _sab_conn = sqlite3.connect(str(MONITOR_DB), check_same_thread=False)
                _sab_conn.row_factory = sqlite3.Row
                rows = _sab_conn.execute(
                ...
                self._send_json({"error": str(_sab_err), "enabled": SHADOW_AB_ENABLED})
            return
            ...
            if should_log and is_messages and input_tokens > 0:
                ...
                if SHADOW_AB_ENABLED and status == 200 and body and not is_sse:
                    try:
                        ...
                        if _sample_pct > 0 and random.random() * 100 < _sample_pct:
