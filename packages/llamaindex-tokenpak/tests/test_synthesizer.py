...
class TestSynthesizerCompression:
    def test_no_compression_when_under_budget(self):
        ...
        assert result["compression_stats"]["input_tokens"] == result["compression_stats"]["output_tokens"]
    ...
    def test_synthesize_returns_query(self):
        s = TokenPakSynthesizer(budget=4000)
        result = s.synthesize("What is TokenPak?", [{"id": "n0", "text": "TokenPak is great.", "metadata": {}, "score": 1.0}])
        assert result["query"] == "What is TokenPak?"

    def test_context_includes_sources(self):
        s = TokenPakSynthesizer(budget=4000)
        nodes = [{"id": "n0", "text": "Evidence text.", "metadata": {"file_name": "paper.pdf"}, "score": 0.9}]
